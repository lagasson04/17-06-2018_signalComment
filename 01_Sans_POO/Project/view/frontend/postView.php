		...
		<p>
			<?php 
				if (isset($comment['report']) && $comment['report'] == 1) {
					echo  '<div class="alert alert-danger" role="alert"><strong>Commentaire signalé !!!</strong></div>';
				}
				else {
					?>
					<p><a href="index.php?action=reportCom&amp;idc=<?=$comment['id'] ?>&amp;idp=<?=$comment['post_id'] ?>"> Signaler !!</a></p>
					<?php
				} ?></p>
			</div>
